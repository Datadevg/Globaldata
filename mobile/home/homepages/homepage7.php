<div class="page-content header-clear">
 ...
 <!-- ASPFIY BANK START-->
                    ...
                    <?php if(empty($data->sAsfiyBank)): ?>
                      <center>  <p class="mb-2 text-danger font-600 font-15"><b>Get A Kuda Account For Automated Transfer, Generate A Dedicated Account Number Now. Funding Attracts</b> <?php echo $asfiyChargesText; ?> only.</p></center>
                      </br> </hr>
                       <center> <p class="mb-2 text-danger font-600 font-15"><font color="green"><b> Kuyi Generate kuda bank account Number domin tura kudi automatic da kuma saukin charges <?php echo $asfiyChargesText; ?> only.</b></font></p></center>
                        
                        
                        <form method="POST" id="kudaform"><input type="hidden" name="generate-aspfiy-account" value="YES" /></form>
                        
                        
               <center> <button class="btn btn-primary font-700 rounded-xl mt-3" id="kudabtn" onclick="$('#kudabtn').removeClass('btn-primary'); $('#kudabtn').addClass('btn-secondary'); $('#kudabtn').html('<i class=\'fa fa-spinner fa-spin\'></i> Processing ...'); $('#kudaform').submit();">Generate Account</button></center>
